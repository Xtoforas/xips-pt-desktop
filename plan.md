...
- [x] Add a Windows helper script that pulls the latest repo state, builds the desktop client with the Windows Tauri toolchain, and launches the built app.
...
- [x] Make `npx tauri build` and `npx tauri dev` rebuild or serve the current desktop frontend automatically so release binaries do not package stale `dist` assets.
- [x] Run Tauri frontend build hooks from `apps/desktop` itself so Windows packaging does not resolve `npm --prefix ..` to a non-existent `apps/package.json`.
