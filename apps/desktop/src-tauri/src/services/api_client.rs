...
impl std::fmt::Display for ApiError {
  fn fmt(&self, formatter: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
    if self.request_id.is_empty() {
      ...
      write!(formatter, "{} ({}) [requestId={}]", self.code, self.detail, self.request_id)
    }
  }
}
...
pub async fn fetch_formats(base_url: &str) -> Result<Vec<TournamentFormat>, String> {
  let response = send_json::<FormatsResponse>(
    &Client::new(),
    Method::GET,
    &format!("{}/api/v1/formats?gameVersion=ootp27", base_url.trim_end_matches('/')),
    None,
    None,
  )
  .await
...
pub async fn fetch_me(base_url: &str, access_token: &str) -> Result<ApiResponse<MeResponse>, ApiError> {
...
pub async fn check_duplicate(
  ...
  send_json::<DuplicateCheckResponse>(
    &Client::new(),
    Method::POST,
    &format!("{}/api/v1/my/uploads/check-duplicate", base_url.trim_end_matches('/')),
    Some(access_token),
    Some(body),
  )
  .await
...
async fn send_json<T: DeserializeOwned>(
  ...
  if let Some(payload) = body {
    builder = builder.header(header::CONTENT_TYPE, "application/json").json(&payload);
  }
...
async fn parse_response<T: DeserializeOwned>(response: Response) -> Result<ApiResponse<T>, ApiError> {
  ...
  if (200..300).contains(&status) {
    ...
    return Ok(ApiResponse { payload, request_id });
  }
...
pub fn into_session_user(payload: &crate::models::api::ApiSessionUser) -> crate::models::local_state::SessionUser {
